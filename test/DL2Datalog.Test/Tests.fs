...
let ``Simplest owl axiom to datalog`` () =
    ...
    let subjectIndex = tripleTable.AddResource(Ingress.Iri(new IriReference "http://example.com/subject"))
    let predIndex = tripleTable.AddResource(Ingress.Iri(new IriReference (Namespaces.RdfType)))
    let concept1Iri = new IriReference "http://example.org/concept1"
    let objdIndex = tripleTable.AddResource(Ingress.Iri concept1Iri)
    let concept2Iri = new IriReference "http://example.org/concept2"
    let objdIndex2 = tripleTable.AddResource(Ingress.Iri(concept2Iri))
    ...
    let X_concept1 = tripleTable.AddResource(Resource.DLTranslatedConceptName concept1Iri)
    let X_concept2 = tripleTable.AddResource(Resource.DLTranslatedConceptName concept2Iri)
...
let  ``Can create axiom rules`` () =
    ...
    let X_subConcept = tripleTable.AddResource(Resource.DLTranslatedConceptName(subConceptIri))
    let superConcept = ALC.ConceptName(superConceptIri)
    let X_superConcept = tripleTable.AddResource(Resource.DLTranslatedConceptName superConceptIri)
