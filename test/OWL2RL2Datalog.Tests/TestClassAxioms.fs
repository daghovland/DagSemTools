...
let ``Subclass RL reasoning from rdf works`` () =
    ...
    let subClassIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference (Namespaces.RdfsSubClassOf)))
    let objIndex2 = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/object2"))
...
let ``Equivalentclass RL reasoning from rdf works`` () =
    ...
    let rdfTypeIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference (Namespaces.RdfType)))
    let objIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/object"))
    let eqClassIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference (Namespaces.OwlEquivalentClass)))
    let classTypeIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference (Namespaces.OwlClass)))
    let objIndex2 = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/object2"))
