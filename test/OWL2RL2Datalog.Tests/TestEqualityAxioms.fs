...
module DagSemTools.OWL2RL2Datalog.TestEqualityAxioms 
    ...
    let ``Equality RL adds equality axioms`` () =
        ...
        let predIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference (Namespaces.OwlSameAs)))
    ...
    let ``Predicate variables are ordered`` () =
        ...
        let objextIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/object"))
    ...
    let ``Predicate variables are handled`` () =
        ...
        let predIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference ("http://example.com/predicate")))
        let objextIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/object"))
        let objextIndex2 = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/object2"))
        let Triple = {Ingress.Triple.subject = subjectIndex; predicate = predIndex; obj = objextIndex}
        tripleTable.AddTriple(Triple)
    ...
    let ``Equality axioms are handled`` () =
        ...
        let objectIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/object"))
        let subjectIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/subject"))
        let sameAsIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference (Namespaces.OwlSameAs)))
        let predIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/predicate"))
    ...
    let ``Equality RL reasoning works`` () =
        ...
        let sameAsIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference (Namespaces.OwlSameAs)))
        let subjectIndex2 = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/subject2"))
        let objIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/object"))
        let predIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/predicate"))
