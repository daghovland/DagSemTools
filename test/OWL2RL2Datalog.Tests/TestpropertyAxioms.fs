...
let ``Object Property Domain and Range RL reasoning works`` () =
    ...
    let propertyIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/property"))
    let rdfTypeIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference (Namespaces.RdfType)))
    let objPropTypeIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference (Namespaces.OwlObjectProperty)))
    let rangeIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/rangeClass"))
    let domainIndex = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference "http://example.com/domainClass"))
    let owlDomainId = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference (Namespaces.RdfsDomain)))
    let owlRangeId = tripleTable.AddResource(Ingress.GraphElement.Iri(new IriReference (Namespaces.RdfsRange)))
