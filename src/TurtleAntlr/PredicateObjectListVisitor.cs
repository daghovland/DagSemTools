...
public class PredicateObjectListVisitor : TurtleBaseVisitor<Func<uint, List<Ingress.Triple>>>
{
        ...
        if (rdfobj.annot != null && rdfobj.annot.children != null)
        {
            var reifications = rdfobj.annot.children
                .Aggregate(seed: new AnnotationStatus(_resourceVisitor.Datastore.NewAnonymousBlankNode(), new List<Ingress.Triple>(), new List<uint>()),
                    ...
                    {
                        TurtleParser.PredicateObjectListContext predobj => HandlePredicateObjectReification(predobj, aggr),
                        TurtleParser.ReifierContext reif => HandleReification(reif, rdfobj.triple, aggr),
                        _ => aggr
                    });
    ...
    private AnnotationStatus HandlePredicateObjectReification(TurtleParser.PredicateObjectListContext predobj, AnnotationStatus aggr)
    {
    ...
    private AnnotationStatus HandleReification(TurtleParser.ReifierContext reif, Ingress.Triple triple, AnnotationStatus aggr)
    {
